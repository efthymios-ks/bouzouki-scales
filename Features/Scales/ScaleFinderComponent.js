...
(() => {
  ...
  const displayNamesCheckbox = container.querySelector(
    `#${prefix}-display-names-label`
  );
  ...
  function renderResults(scales) {
    ...
    for (const scale of scales) {
      ...
      const notes = scale.normalizedNotes.map(getNoteName).join(" ");
      const scaleOtherNamesHtml =
        scale.otherNames && scale.otherNames.length
  ...
  function updateSelectionInfo() {
    const selectedCount = notesContainer.querySelectorAll(
      `.${prefix}-note.active`
    ).length;
  ...
  function renderNotes() {
    ...
    Scale.getAllTonics().forEach((noteKey) => {
      ...
      noteElement.addEventListener("click", () => {
        ...
        const activeNotes = notesContainer.querySelectorAll(
          `.${prefix}-note.active`
        );
